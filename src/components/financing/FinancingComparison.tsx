...
export default function FinancingComparison() {
  ...
  const [showFinanciamentoForm, setShowFinanciamentoForm] = useState(false);
  const [selectedFinanciamento, setSelectedFinanciamento] = useState(null);
  const [selectedBankForSimulation, setSelectedBankForSimulation] = useState(null);
  ...
  const handleSolicitarSimulacao = (banco: any) => {
    setSelectedBankForSimulation(banco);
    toast({
      title: "Simulação Iniciada",
      description: `Simulação pré-configurada para ${banco.banco}. Vá para a aba Simulador para continuar.`,
    });
  };

  const handleSolicitarProposta = (resultado: any) => {
    // Criar dados do financiamento baseado na simulação
    const novoFinanciamento = {
      banco: resultado.banco,
      valor_financiado: resultado.valorFinanciado,
      entrada: parseFloat(simulacao.entrada),
      prazo_meses: parseInt(simulacao.prazo),
      taxa_juros: resultado.taxaMin,
      valor_parcela: resultado.parcela,
      status: 'analise'
    };
    setSelectedFinanciamento(novoFinanciamento);
    setShowFinanciamentoForm(true);
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="comparacao" className="w-full">
                ...
                {rates.map((rate, index) => (
                  <div key={rate.codigo} className="border rounded-lg p-4">
                    ...
                    <div className="flex justify-between items-center mt-3">
                      ...
                      <Button size="sm" variant="outline" onClick={() => handleSolicitarSimulacao(rate)}>
                        Solicitar Simulação
                      </Button>
                    </div>
                  </div>
                    ...
                    <div key={resultado.codigo} className={`border rounded-lg p-4 ${
                      ...
                      <div className="flex justify-end mt-3">
                        <Button 
                          ...
                          onClick={() => resultado.atendeCriterios && handleSolicitarProposta(resultado)}
                        >
        ...
        <TabsContent value="gestao" className="space-y-6">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xl font-semibold">Gestão de Financiamentos</h3>
              <p className="text-muted-foreground">Acompanhe suas solicitações e processos</p>
            </div>
            <Button onClick={() => setShowFinanciamentoForm(true)}>
      ...
      <Dialog open={showFinanciamentoForm} onOpenChange={setShowFinanciamentoForm}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selectedFinanciamento?.id ? 'Editar Financiamento' : 'Cadastrar Novo Financiamento'}
            </DialogTitle>
          </DialogHeader>
          <FinanciamentoForm 
            financiamento={selectedFinanciamento}
            onClose={() => {
              setShowFinanciamentoForm(false);
